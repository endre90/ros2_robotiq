...
class Window(QWidget, CommVariables):
    def __init__(self, parent=None):
        ...
        if self.namespace == '':
            self.setWindowTitle("Robotiq2F joint pose controller")
        else:
            self.setWindowTitle("Robotiq2F joint pose controller - " + self.namespace)
        self.resize(550, 250)

class Ros2Robotiq2FGui(Node, CommVariables):

    def __init__(self):
        super().__init__("ros2_robotiq_2f_gui")
        ...
        self.gui_to_utils_msg = Robotiq2FGuiToUtils()
...
def main(args=None):
    ...
    def launch_node():
        def launch_node_callback_local():
            ...
            ros2_robotiq_2f_gui = Ros2Robotiq2FGui()
            rclpy.spin(ros2_robotiq_2f_gui)
            ros2_robotiq_2f_gui.destroy_node()
            rclpy.shutdown()
